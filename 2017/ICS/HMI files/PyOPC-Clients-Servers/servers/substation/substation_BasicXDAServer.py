...
class BasicXDAServer(XDAServer):
	...
	relay_address = paths['pw_relay']
	gen_address = paths['generator']
	xda_relay = XDAClient(OPCServerAddress=relay_address)
	xda_gen = XDAClient(OPCServerAddress=gen_address)
        newIPH = ItemPairHolder()
        for inItem, outItem in IPH:
            key = mkItemKey(inItem)
            if self.OPCItemDict.has_key(key):
		i_name = self.OPCItemDict[key].ItemName
		if(i_name == "breaker" or i_name == "flow" or i_name == "load"):
			resp = (xda_relay.Read(ItemContainer(ItemName = self.OPCItemDict[key].ItemName )))
			self.OPCItemDict[key].Value = resp[0][0].Value;
		if(i_name == "generation"):
			resp = (xda_gen.Read(ItemContainer(ItemName = self.OPCItemDict[key].ItemName )))
			self.OPCItemDict[key].Value = resp[0][0].Value;
	...
	xda_gen = XDAClient(OPCServerAddress=gen_address)
        for inItem,outItem in IPH:
	    i_name = inItem.ItemName
	    if(i_name == "breaker" or i_name == "flow" or i_name == "load"):
	    	xda_relay.Write([inItem],LocaleID='en-us')
	    if(i_name == "generation"):
	    	xda_gen.Write([inItem],LocaleID='en-us')
            key = mkItemKey(inItem)
