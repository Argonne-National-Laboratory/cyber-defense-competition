...
class BasicXDAServer(XDAServer):
	...
	address = config_info['cimore_address']
	xda = XDAClient(OPCServerAddress=address)
		...
		resp = (xda.Read(ItemContainer(ItemName = self.OPCItemDict[key].ItemName )))
		self.OPCItemDict[key].Value = resp[0][0].Value;
		print self.OPCItemDict[key];
		#print outItem[0]
		#outItem.Value=resp[0][0].Value
		#outItem = resp[0][0]
	...
	xda = XDAClient(OPCServerAddress=address,ReturnErrorText=True)
        for inItem,outItem in IPH:
	    print xda.Write([inItem],LocaleID='en-us')
            key = mkItemKey(inItem)
