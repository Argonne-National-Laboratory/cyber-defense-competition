...
class Resevoir:
	...
	def Draw(self, canvas):
		self.resevoir = canvas.AddRectangle((self.pos[0], self.pos[1]-self.height+self.ellip_height/2), (self.width+.1, self.height), FillColor=wtc.TANK_BACKGROUND_COLOR, InForeground=True)
		canvas.AddEllipse((self.pos[0], self.pos[1]-self.height), (self.width, self.ellip_height), FillColor=wtc.TANK_BACKGROUND_COLOR, InForeground=True)
		canvas.AddEllipse(self.pos, (self.width, self.ellip_height), FillColor=wtc.TANK_BACKGROUND_COLOR, InForeground=True)
		if self.text:
...
class WaterTower:
	def __init__(self, pos=(0,0), scale=1, height=10, width=10):
		self.pos = pos
		self.scale = scale
		self.height = height * self.scale
		self.ellip_height = height * self.scale / 2
	...
	def Draw(self, canvas):
		self.base = canvas.AddRectangle((self.pos[0]+9, self.pos[1]-50), (3, 50), FillColor=wtc.TANK_BACKGROUND_COLOR)
		self.reservoir = Resevoir(self.pos, text='', scale=self.scale, height=10)
		self.reservoir.Draw(canvas)
		self.fill = canvas.AddRectangle((self.pos[0], self.pos[1]-self.height+self.ellip_height/2), (self.width+.1, self.height-9), FillColor='blue', InForeground=True)
		self.base_tank = canvas.AddEllipse((self.pos[0], self.pos[1]-self.height), (self.width, self.ellip_height), FillColor='blue', InForeground=True)
		self.fill2 = canvas.AddEllipse((self.pos[0], self.pos[1]-8), (self.width, self.ellip_height-2.5), FillColor='blue', InForeground=True)
